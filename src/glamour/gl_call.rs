...
/// A wrapper for an unsafe OpenGL call that returns a `Result`.
///
/// Returns `Err` if any OpenGL errors exist.
///
/// FIXME: This is basically useless unless all errors are cleared beforehand.
/// TODO: create an error type for OpenGL errors for matching.
///
/// # Examples
///
/// ```no_run
...
/// #
/// let mut id = 0;
/// gl_try!(gl::GenBuffers(1, &mut id)).expect("Failed to generate buffer");
/// assert_ne!(id, 0);
/// ```
#[macro_export]
macro_rules! gl_try {
    ($gl_fn:expr) => {{
        let mut result = Ok(unsafe { $gl_fn });
        let mut errors = Vec::<String>::new();
        while let Some(error) = $crate::gl_call::gl_get_error() {
            errors.push($crate::gl_call::gl_get_error_name(&error));
        }
        if !errors.is_empty() {
